...
const populateRandomAmenities = (num) => {
  const newAmenity = defaultAmenities.slice();
  const allAmenities = AMENITY_DATA.slice();
...
const createAmenityCsv = () => {
  for (let i = 1; i <= 10000000; i += 1) {
    const randomAmenities = populateRandomAmenities(faker.random.number({ min: 2, max: 6 }));
    for (let x = 0; x < randomAmenities.length; x += 1) {
      const amenity = randomAmenities[x];
...
const createRooms = () => {
  ...
  for (let i = 1; i <= 10000000; i += 1) {
    ...
    for (let j = 0; j < cancellationsLength; j += 1) {
      cancelationRules.push(faker.lorem.words(faker.random.number({ min: 3, max: 5 })));
    }
    ...
    for (let x = 0; x < dataItem.beds; x += 1) {
      let bedDetails = '';
    ...
    const dataCategories = [dataItem.room_id, dataItem.roomName, dataItem.city,
      dataItem.type, dataItem.title, dataItem.max_guest, dataItem.subtype,
      dataItem.beds, dataItem.baths, dataItem.host_username, dataItem.avatar,
      JSON.stringify(dataItem.highlights).replace(new RegExp(',', 'g'), '*'),
      dataItem.short_description, dataItem.main_description,
      JSON.stringify(dataItem.house_rules).replace(new RegExp(',', 'g'), '*'),
      dataItem.house_rules_description,
      JSON.stringify(dataItem.cancellations).replace(new RegExp(',', 'g'), '*'),
      JSON.stringify(dataItem.sleeping_arrangements).replace(new RegExp(',', 'g'), '*')];
    const csvFormatData = dataCategories.join();
